...
final class MBAPIRequestor: MBEndpointRequesting, Sendable {
    ...
    func send<Response: Decodable, Body: Encodable>(
        ...
        do {
            ...
            await logIssue(
                ...
                context: context,
                responseType: responseTypeName,
                errorDescription: "Request body encoding failed: \(error.localizedDescription)"
            )
            throw MBClientError.decodingFailed("Request body encoding failed: \(error.localizedDescription)")
        }
    ...
    func uploadMultipart<Response: Decodable>(
        ...
        request.setValue("Bearer \(context.accessToken)", forHTTPHeaderField: "Authorization")
        request.httpBody = multipartBody(file: file, boundary: boundary)
        ...
        do {
            ...
            let responsePreview = String(data: data, encoding: .utf8)?
                .prefix(responseBodyLogLength) ?? "<non-UTF8 body>"
            logger.error("Multipart decode failed for responseType=\(String(describing: responseType), privacy: .public)")
            logger.error("Request URL: \(requestURL.absoluteString, privacy: .public)")
            logger.error("Status: \(httpResponse.statusCode)")
            logger.error("Error: \(error.localizedDescription, privacy: .public)")
            logger.error("Body preview: \(String(responsePreview), privacy: .public)")
    ...
    private func multipartBody(file: MBMultipartFile, boundary: String) -> Data {
        var data = Data()
